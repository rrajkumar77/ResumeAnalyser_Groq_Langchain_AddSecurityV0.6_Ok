...
with st.sidebar:
    ...
    if 'skill_templates' in st.session_state and st.session_state.skill_templates:
        ...
        for template_name in st.session_state.skill_templates.keys():
            ...
            with col_a:
                if st.button(f"📂 {template_name}", key=f"load_{template_name}"):
...
with col1:
    ...
    if skill_mode == "Auto-extract from JD":
        ...
        st.info("✍️ Enter the top 5 skills you want to validate")
        
        manual_skills = []
        
        st.subheader("🎯 Enter Top 5 Skills")
        ...
        if len(manual_skills) == 5:
            ...
            st.warning(f"⚠️ Please enter all 5 skills ({len(manual_skills)}/5 entered)")
        else:
            st.info("👆 Enter 5 skills above to continue")
