...
class JDContext:
    ...
    required_skills: List[str] = field(default_factory=list)  # ✨ NEW: Extracted from JD
...
class JDContextParser:
    ...
    def parse_jd(self, jd_text: str) -> JDContext:
        ...
        # ✨ NEW: Extract required/must-have skills from JD
        context.required_skills = self._extract_required_skills(jd_text)
    ...
    def _extract_required_skills(self, jd_text: str) -> List[str]:
        """Extract required/must-have skills from JD"""
        skills = []
        
        # Patterns to find skill sections in JD
        ...
        for pattern in skill_section_patterns:
            ...
            for match in matches:
                ...
                for item in items:
                    skill = item.strip()
                    # Clean up (remove years, parentheses, etc.)
                    skill = re.sub(r'\(.*?\)', '', skill)  # Remove parentheses
                    skill = re.sub(r'\d+\+?\s*years?', '', skill, flags=re.IGNORECASE)  # Remove year requirements
                    skill = skill.strip()
                    
                    # Only keep if valid length
...
"""
    ...
    for i, skill in enumerate(sorted_validated[:5], 1):
        ...
        # Add outcome indicators - FIX: enhanced_evidence is a LIST
        if hasattr(skill, 'enhanced_evidence') and skill.enhanced_evidence:
            has_outcomes = False
            if isinstance(skill.enhanced_evidence, list):
                # Check each piece of evidence for outcome indicators
                for evidence in skill.enhanced_evidence:
                    evidence_text = getattr(evidence, 'evidence_text', '')
                    if evidence_text:
                        evidence_lower = evidence_text.lower()
                        if any(indicator in evidence_lower for indicator in ['reduced', 'increased', 'improved', 'achieved', '%', 'revenue', 'cost']):
                            has_outcomes = True
                            break
...
with tab1:
    ...
    if st.session_state.jd_context:
        with st.expander("🔍 JD Context Analysis", expanded=True):
            ctx = st.session_state.jd_context
            if ctx.primary_role_type:
                st.info(f"**Primary Role Type:** {ctx.primary_role_type.replace('_', ' ').title()}")
            
            # ✨ NEW: Show extracted required skills from JD
            if ctx.required_skills:
                st.success(f"**✅ Extracted Required Skills from JD:** ({len(ctx.required_skills)} skills)")
                skills_display = ", ".join(ctx.required_skills[:15])  # Show first 15
                if len(ctx.required_skills) > 15:
                    skills_display += f" ... and {len(ctx.required_skills) - 15} more"
                st.write(skills_display)
                
                # Option to auto-populate priority skills
                if not st.session_state.locked_priority_skills:
                    if st.button("🎯 Use these as Priority Skills"):
                        st.session_state.locked_priority_skills = ctx.required_skills[:10]  # Top 10
                        st.rerun()
    ...
    if analyze_clicked or gen_summary:
        if not jd_text or not resume_text:
            ...
            with st.spinner("🔍 Analyzing candidate fit..."):
                ...
                # ✨ NEW: Merge JD-extracted skills with user-provided priority skills
                all_priority_skills = priority_skills.copy()
                if st.session_state.jd_context and st.session_state.jd_context.required_skills:
                    # Add JD-extracted skills that aren't already in priority list
                    for skill in st.session_state.jd_context.required_skills:
                        if skill.lower() not in [s.lower() for s in all_priority_skills]:
                            all_priority_skills.append(skill)
                
                # Show what we're analyzing for
                if len(all_priority_skills) > len(priority_skills):
                    st.info(f"ℹ️ Analyzing {len(all_priority_skills)} skills: {len(priority_skills)} user-specified + {len(all_priority_skills) - len(priority_skills)} from JD")
                
                # Analyze
                report = semantic_matcher.analyze_with_priorities(
                    jd_text=jd_masked,
                    resume_text=resume_masked,
                    priority_skills=all_priority_skills  # ✨ Use merged skills
                )
