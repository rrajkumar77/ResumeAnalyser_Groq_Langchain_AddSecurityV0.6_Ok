...
"""
    ...
    for i, skill in enumerate(sorted_validated[:5], 1):
        ...
        if hasattr(skill, 'enhanced_evidence') and skill.enhanced_evidence:
            ev_text = to_text(skill.enhanced_evidence)
            if any(indicator in ev_text.lower() for indicator in ['reduced', 'increased', 'improved', 'achieved', '%', 'revenue', 'cost']):
...
def display_skill_card(skill, is_priority: bool, jd_context: JDContext = None):
    ...
    with st.expander(
        ...
        # Check for metrics
        has_metrics = False
        evidence_text = to_text(getattr(skill, 'enhanced_evidence', ""))
